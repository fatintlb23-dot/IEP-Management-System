...
<style>
    ...
    .table-container table tbody td .student-name {
        font-weight: 600;
        color: #0B1E33;
        text-transform: uppercase;
    ...
    .table-container table tbody td .student-name:hover {
        color: #4F7CFF;
    }

    .table-container table tbody td .student-ic {
        font-size: 13px;
        color: #94A3B8;
    }

    .table-container table tbody td .diagnosis-badge {
    ...
    .table-container table tbody td .view-link {
        display: inline-flex;
        align-items: center;
        gap: 6px;
        padding: 6px 14px;
    ...
    .table-container table tbody td .view-link:hover {
        background: #DBEAFE;
    }
...
<div class="page-wrapper">
    ...
    <div class="hero-banner">
        ...
        <div class="hero-left">
            <div class="hero-label">Student Management</div>
            <h1>View All <span>Students</span></h1>
            <div class="hero-subtitle">View and manage all students in the system.</div>
        </div>
    ...
    <div class="search-filter-bar">
        <div class="search-input-wrapper">
            ...
            <input type="text" id="studentSearch" placeholder="Search by name, IC, or diagnosis..." onkeyup="searchStudents()">
        </div>
    ...
    <div class="table-container">
        <table>
            ...
            <tbody id="studentTableBody">
                ...
                <tr class="student-row" data-teacher="{{ $student->teacher_id }}" data-name="{{ strtolower($student->student_name) }}" data-ic="{{ $student->student_ic }}" data-diagnosis="{{ strtolower($student->student_diagnosis) }}">
                    <td>{{ $students->firstItem() + $index }}</td>
                    <td>
                        <a href="{{ route('admin.students.show', $student->student_id) }}" style="text-decoration: none; color: inherit;">
                            <div class="student-name">{{ strtoupper($student->student_name) }}</div>
                        </a>
                    </td>
                    <td><span class="student-ic">{{ $student->student_ic }}</span></td>
                    <td>
                        <span class="diagnosis-badge">{{ $student->student_diagnosis }}</span>
                    </td>
                    <td>{{ $student->student_age }}</td>
                    <td>{{ $student->teacher ? strtoupper($student->teacher->name) : '-' }}</td>
                    <td>{{ $student->parent ? strtoupper($student->parent->name) : '-' }}</td>
                    <td>
                        <a href="{{ route('admin.students.show', $student->student_id) }}" class="view-link">
                            View
                        </a>
                    </td>
                </tr>
        ...
        <div class="pagination-section">
            <div class="pagination-info">
                Showing <strong id="showingStart">{{ $students->firstItem() ?? 0 }}</strong> to <strong id="showingEnd">{{ $students->lastItem() ?? 0 }}</strong> of <strong id="totalCount">{{ $students->total() }}</strong> students
            </div>
...
<script>
    function searchStudents() {
        ...
        rows.forEach(row => {
            ...
            const matchesSearch = name.includes(filter) || ic.includes(filter) || diagnosis.includes(filter);
