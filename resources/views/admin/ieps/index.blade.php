    ...
    <div class="table-container">
        <table>
            ...
            <tbody id="studentTableBody">
                ...
                @endphp
                <tr class="student-row" data-teacher="{{ $student->teacher_id }}" data-name="{{ strtolower($student->student_name) }}" data-ic="{{ $student->student_ic }}">
                    <td>{{ $students->firstItem() + $index }}</td>
                    <td>
                        <a href="{{ route('admin.students.show', $student->student_id) }}" style="text-decoration: none; color: inherit;">
                            <div class="student-info">
                                <span class="student-name">{{ strtoupper($student->student_name) }}</span>
                            </div>
                        </a>
                    </td>
                    <td>
                    ...
                    <td>
                        <div style="display: flex; flex-wrap: wrap; gap: 4px; align-items: center;">
                            @foreach($uniqueCategories as $category)
                                <span class="goal-tag {{ strtolower($category) }}">{{ $category }}</span>
                            @endforeach
                            @if($goalCount > 0)
                                <span style="font-size: 12px; color: #94A3B8;">({{ $goalCount }} goals)</span>
                            @else
                    ...
                    <td>
                        @if($avgProgress > 0)
                            <a href="{{ route('admin.students.show', $student->student_id) }}" class="progress-link">
                                <div class="progress-bar">
                                    <div class="progress-fill {{ $progressColor }}" style="width: {{ $avgProgress }}%; min-width: 4px;"></div>
                                </div>
                                <span class="progress-text" style="color: {{ $progressTextColor }};">
                                    {{ $avgProgress }}%
                                </span>
                            </a>
                        @else
